...
// it. Only the video decrypt path below actually needs them.
let _key = null;
let _iv = null;
...
module.exports.toggleGamificationStatus = catchAsync(async (req, res, next) => {
  ...
  // to say "deactivate".
  if (!gamification_id || status === undefined || status === null || status === "") {
  ...
  const given =
    typeof status === "boolean" ? String(status) : String(status).trim().toLowerCase();
...
module.exports.createMultipleGamificationQst = catchAsync(
  async (req, res, next) => {
    ...
    // A JSON body already arrives parsed, so the previous JSON.parse(questions)
    // threw on every valid request. Only parse when the client actually sent a
    // string, which multipart/form-data clients do.
    if (typeof questions === "string") {
      try {
        questions = JSON.parse(questions);
      } catch {
        return next(new AppError("questions is not valid JSON", 400));
      }
    }
