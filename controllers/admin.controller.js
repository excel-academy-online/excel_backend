...
module.exports.AdminLogin = catchAsync(async (req, res, next) => {
  ...
  // Signing in proves who you are, not that you are staff. Without this check
  // any student could sign in here and be handed an admin session cookie.
  // Authority is the Firebase custom claim - the same source middleware/auth.js
  // trusts - with the legacy Firestore `admin` flag accepted as a fallback for
  // accounts created before roles existed.
  const assertAdmin = async (uid, userData) => {
    const { auth: adminAuth, ADMIN_ROLES } = (() => {
      const { auth } = require("../firebaseadminvar");
      const { ADMIN_ROLES } = require("../middleware/auth");
      return { auth, ADMIN_ROLES };
    })();

    let role = null;
    try {
      const record = await adminAuth.getUser(uid);
      role = (record.customClaims || {}).role || null;
    } catch (err) {
      console.warn("Could not read custom claims for", uid, err.message);
    }
    ...
    if (userData && userData.admin === true) {
      console.warn(
        `Admin ${uid} has no role claim; falling back to the Firestore flag. ` +
          "Run: npm run set:role -- <email> admin"
      );
      return true;
    }
    return false;
  ...
  signInWithEmailAndPassword(auth, email, password)
    .then(async (userCredential) => {
      ...
      if (idToken) {
        ...
        // Refuse non-staff before handing out a session.
        if (!(await assertAdmin(uid, userData))) {
          res.clearCookie("access_token");
          return next(
            new AppError("This account does not have administrator access", 403)
          );
        }
