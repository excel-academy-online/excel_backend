...
// Reads courses from MongoDB or Firestore depending on DATA_SOURCE.
...
module.exports.InitializePayment = catchAsync(async (req, res, next) => {
  ...
  const coursesFetched = await courseRepo.findManyByIds(product_ids);
  ...
  // The client-supplied amount is untrusted; it only has to agree with the
  // total we computed from the database. Compare as integers in kobo so that
  // a string "5000" and prices like 1999.99 both behave.
  const submittedKobo = Math.round(Number(amount) * 100);
  const expectedKobo = Math.round(initialAmount * 100);

  if (!Number.isFinite(submittedKobo) || submittedKobo !== expectedKobo) {
    return next(
      new AppError(
        `Payment failed because the amount did not match. Please pay ${initialAmount} Naira`,
        402
      )
    );
  }
  ...
  const apiReq = https.request(options, (apiRes) => {
    ...
    apiRes.on("end", async () => {
      ...
      if (result.status != false && result.data.status === "success") {
        ...
        const courseFetched = await courseRepo.findManyByIds(product_ids);
        ...
        // Check for a repeat purchase BEFORE mutating the user, otherwise the
        // courses are appended and only then rejected.
        // A Firestore document may simply not have the field yet, whereas the
        // Mongoose schema always defaulted it to an array.
        const isSubscriber = courseFetched.some((course) =>
          (course.subscribers || []).some((s) => String(s) === String(subscriber_id))
        );
        ...
        // Record the buyer against each course. forEach(async) does not wait,
        // so these writes have to be collected and awaited together.
        await Promise.all(
          courseFetched.map((course) =>
            courseRepo.addSubscriber(course, subscriber_id)
          )
        );
        await user.save();
