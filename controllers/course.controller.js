...
// startup whenever the env vars were absent, taking every unrelated route with
// it. Only the video encrypt/decrypt paths actually need them.
let _key = null;
let _iv = null;

function getCryptoParams() {
  if (_key && _iv) return { key: _key, iv: _iv };

  if (!process.env.key || !process.env.iv) {
    throw new AppError(
      "Video encryption is not configured on this server (set key and iv)",
      500
    );
  }
  ...
  if (key.length !== 32) {
    throw new AppError("Encryption key must be 32 bytes (64 hex chars)", 500);
  }
  if (iv.length !== 16) {
    throw new AppError("Encryption IV must be 16 bytes (32 hex chars)", 500);
  }
...
module.exports.GetCourseDetails = catchAsync(async (req, res, next) => {
  ...
  const findCourseById = await courseRepo.findById(courseId);
  ...
  // Lessons are embedded in the course document, so on Firestore they are
  // already present. On Mongo the nested module refs still need populating.
  let innerCourseId = findCourseById;
  if (courseRepo.source === "mongo" && typeof findCourseById.populate === "function") {
    try {
      innerCourseId = await findCourseById.populate({
        path: "lessons",
        populate: { path: "modules" },
      });
    } catch (err) {
      // "lessons" is an embedded array rather than a ref, so this populate
      // has always been liable to throw. Fall back to the unpopulated
      // document instead of failing the whole request.
      console.warn("Could not populate lessons:", err.message);
    }
  }
